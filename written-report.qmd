...
Given the fact that none of the additional variables assessed were statistically significant, our final model is the same as our original model with university, race, first-generation status, threshold gpa, and total sleep time being predictors of whether a GPA was above the 75th percentile or not.

The odds of having a GPA above the 75th percentile for a student at the catholic private school are 1.842 times lower than that of a student at the public university, holding all else constant.

The odds of having a GPA above the 75th percentile for a student at the stem private school are 1.202 times higher than that of a student at the public university, holding all else constant. The p-value for the stem private school is 0.483, which is not significant, so the interpretation of these results is not significant.

The odds of having a GPA above the 75th percentile for a non-underrepresented student are 2.298 times lower than that of a underrepresented student, holding all else constant.

The odds of having a GPA above the 75th percentile for a first generation student are 2.748 times higher than that of a non-first generation student, holding all else constant.
...
For each minute increase in `TotalSleepTime` The odds of having a GPA above the 75th percentile are multiplied by a factor of 0.992 (0.8% decrease) , holding all else constant.
...
Based on the results from previous studies, we are aware that sleeping more will have a positive effect on GPA. However, our results show that the odds of having a GPA above the 75th percentile decrease slightly as total sleep time increases. We hypothesize that while sleeping more does have a generally positive effect on GPA, sleeping too much may have a negative effect on GPA, which is pulling down the coefficient for total sleep time.

Based on the results from the coefficient for threshold GPA, we noticed that if you have previously done well, i.e., your first semester GPA was already above the 75th percentile, it is likely that in the following semester, you will have a lower GPA, because it is more difficult to maintain a high GPA, or improve upon it, compared to if your first semester GPA was below the 75th percentile. This could explain why students with a lower term GPA than their cumulative GPA have a much higher odds of being above the 75th percentile in term GPA.
...
\*We would recommend institution administrators to create programs that target \_\_\_\_?
