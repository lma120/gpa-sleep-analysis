...
grid.arrange(p8, p7, p5, p12, ncol = 4)
...
Model 2: as.factor(gpa_split) \~ university + demo_race + threshold_gpa + TotalSleepTime + daytime_sleep_lvl
...
ufit4_anova <- anova(university_fit1, university_fit4, test = "Chisq")
...
We then decided to check the interaction effects of main effects that were significant for daytime_sleep_lvl and university because \_\_\_\_\_\_\_\_\_.

**Analysis of Deviance Table**

Model 1: as.factor(gpa_split) \~ university + demo_race + threshold_gpa + TotalSleepTime + daytime_sleep_lvl

Model 2: as.factor(gpa_split) \~ university + demo_race + threshold_gpa + TotalSleepTime +daytime_sleep_lvl + daytime_sleep_lvl\*TotalSleepTime

```{r}
int_fit1 <- glm(as.factor(gpa_split) ~ university + demo_race + threshold_gpa + daytime_sleep_lvl + TotalSleepTime + daytime_sleep_lvl*TotalSleepTime, data = university_clean, family = binomial)


int1_anova <- anova(university_fit4, int_fit1, test = "Chisq")
...
```{r}
int_fit2 <- glm(as.factor(gpa_split) ~ university + demo_race + threshold_gpa + daytime_sleep_lvl + TotalSleepTime + university*TotalSleepTime, data = university_clean, family = binomial)

int2_anova <- anova(university_fit4, int_fit2, test = "Chisq")
...
```{r}
university_woint <- glm(as.factor(gpa_split) ~ university + demo_race + threshold_gpa + TotalSleepTime + daytime_sleep_lvl, data = university_clean, family = binomial)

university_final <- glm(as.factor(gpa_split) ~ university + demo_race + threshold_gpa + TotalSleepTime + daytime_sleep_lvl + university*TotalSleepTime, data = university_clean, family = binomial) #final model!

vif(university_woint) #????

```

DID NOT INCLUDE INTERACTION TERMS TO SEE WHAT MAIN EFFECTS MIGHT BE MULTICOLLINEARITY
...
$$
logit(p_{high\_gpa}) = 2.544 - 2.825 \times universitycath\_priv \\ + 0.276 \times universitystem\_priv \\
- 1.049 \times demo\_race - \\1.036 \times threshold\_gpalow
\\ - 0.0063 \times TotalSleepTime \\- 0.682 \times daytime\_sleep\_lvllow
$$

$$
p_{high\_gpa} = \frac {1} {1+e^{2.544 - 2.825 \times universitycath\_priv + 0.276 \times universitystem\_priv \\
- 1.049 \times demo\_race - 1.036 \times threshold\_gpalow \\
- 0.0063 \times TotalSleepTime - 0.682 \times daytime\_sleep\_lvllow}} 
$$
...
pa5 <- ggplot(data = fin_aug, aes(x = daytime_sleep_lvl, y = final_pred)) + 
...
In conclusion, significant factors that impact academic performance are university, underrepresented status, academic performance compared across semesters, total sleep time, and whether or not students napped for over 1 hour during the day.
